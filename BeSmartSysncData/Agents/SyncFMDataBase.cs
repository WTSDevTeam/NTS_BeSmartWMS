...
namespace BeSmartSysncData.Agents
{
    public class SyncFMData
    {
        ...
        public void ProcessExport(string inDir)
        {
            this.pm1Process(inDir);
        }
        ...
        private void pm1Process(string inDir)
        {
            ...
            string strSQLStr = "select * from SYNCDBDATA order by TDATETIME";
            if (pobjSQLUtil.SQLExec(ref dtsDataEnv, "QSyncDB", "SYNCDBDATA", strSQLStr, ref strErrorMsg))
            {
                ...
                foreach (DataRow dtrSCoor in dtsDataEnv.Tables["QSyncDB"].Rows)
                {
                    ...
                    string strCode = dtrSCoor["TAB_CODE"].ToString().TrimEnd();
                    string strOldCode = dtrSCoor["TAB_OLDCODE"].ToString().TrimEnd();
                    switch (dtrSCoor["TAB_NAME"].ToString().TrimEnd().ToUpper())
                    {
                        case "GLREF":
                            bllResult = pmSyncGLRef(dtrSCoor["cType"].ToString().TrimEnd().ToUpper(), strCode, dtrSCoor["TAB_ROWID"].ToString());
        ...
        private void pmSaveGLRef(DataSet inSource)
        {
            ...
            try
            {
                ...
                this.pmSaveGLRef(inSource, ref strError_Issue);
        ...
        private bool pmSaveGLRef(DataSet inSource, ref string ioErrorMsg)
        {
            bool bllResult = true;
            ...
            ////{
            ////	strRefNo = this.mstrRefType+this.txtQcAcBook.Text.TrimEnd()+"/"+inGLRef["cCode"].ToString().TrimEnd();
            ////}

            //decimal decVATRate = this.mdecVatRate;
            ...
            //strRefNo = inGLRef["cRefNo"].ToString();

            //dtrGLRef["fcSkid"] = this.mstrEditRowID;
            //dtrGLRef["fcCorp"] = App.ActiveCorp.RowID;
            //dtrGLRef["fcBranch"] = inGLRef["cBranch"].ToString();
            //dtrGLRef["fcStep"] = SysDef.gc_REF_STEP_CUT_STOCK;
            //dtrGLRef["fcStat"] = inGLRef["cStatus"].ToString();
            //dtrGLRef["fcAtStep"] = this.mstrAtStep;
            //dtrGLRef["fcRefType"] = this.mstrRefType;
            //dtrGLRef["fcRfType"] = this.mstrRfType;
            //dtrGLRef["fcBook"] = inGLRef["cBook"].ToString();
            //dtrGLRef["fcCode"] = inGLRef["cCode"].ToString().TrimEnd();
            //dtrGLRef["fcCode"] = this.mstrRunCode;
            //dtrGLRef["fcRefNo"] = inGLRef["cRefNo"].ToString();
            //dtrGLRef["fdDate"] = Convert.ToDateTime(inGLRef["dDate"]);
            //dtrGLRef["fdDebtDate"] = Convert.ToDateTime(inGLRef["dDate"]);
            //dtrGLRef["fdReceDate"] = Convert.ToDateTime(inGLRef["dDate"]);

            //if (Convert.IsDBNull(inGLRef["dDueDate"]))
            //{
            //    dtrGLRef["fdDueDate"] = this.pmCalDueDate(Convert.ToDateTime(inGLRef["dDate"]), intCredTerm);
            //}
            //else
            //{
            //    dtrGLRef["fdDueDate"] = Convert.ToDateTime(inGLRef["dDueDate"]);
            //}

            ////dtrGLRef["fdDueDate"] = this.pmCalDueDate(Convert.ToDateTime(inGLRef["dDate"]), intCredTerm);
            //dtrGLRef["fdVATDate"] = Convert.ToDateTime(inGLRef["dDate"]);
            //dtrGLRef["fcSect"] = inGLRef["cSect"].ToString();
            //dtrGLRef["fcDept"] = inGLRef["cDept"].ToString();
            //dtrGLRef["fcJob"] = inGLRef["cJob"].ToString();
            //dtrGLRef["fcProj"] = inGLRef["cProj"].ToString();
            //dtrGLRef["fcVatIsOut"] = inGLRef["cVATIsOut"].ToString();
            //dtrGLRef["fcVatType"] = inGLRef["cVATTypeCode"].ToString();
            //dtrGLRef["fnVatrate"] = Convert.ToDecimal(inGLRef["nVATRate"]); ;
            //dtrGLRef["fcCoor"] = strCoor;
            //dtrGLRef["fcEmpl"] = strEmpl;
            //dtrGLRef["fcRecvMan"] = "";
            //dtrGLRef["fcDeliCoor"] = strCoor;
            //dtrGLRef["fcVatCoor"] = strCoor;
            //dtrGLRef["fcTradeTrm"] = inGLRef["cTradeTerm"].ToString();
            //dtrGLRef["fnCredTerm"] = intCredTerm;
            //dtrGLRef["fcIsCash"] = this.mstrIsCash;
            //dtrGLRef["fcDebtCode"] = inGLRef["cDebtCode"].ToString();
            ////22/07/08 : ฺBy Yod แก้เรื่องขอคืนต้องแสดง Y
            ////dtrGLRef["fcHasRet"] = "";
            //dtrGLRef["fcHasRet"] = this.mstrHasReturn;
            //dtrGLRef["fcVatDue"] = this.mstrVatDue;
            //dtrGLRef["fcShowPled"] = "Y";
            //dtrGLRef["fcDiscStr"] = "";
            //dtrGLRef["fcCurrency"] = inGLRef["cCurr"].ToString();
            //dtrGLRef["fnXRate"] = Convert.ToDecimal(inGLRef["nXRate"]);
            //dtrGLRef["fnAmtKe"] = this.mdecAmtK;
            //dtrGLRef["fnVatAmtKe"] = this.mdecVatAmtK;
            ////dtrGLRef["fnDiscAmtK"] = Convert.ToDecimal(inGLRef["nDiscAmt"]);
            ////dtrGLRef["fnDiscAmt1"] = Convert.ToDecimal(inGLRef["nDiscAmt"]);

            //dtrGLRef["fnDiscAmtK"] = this.mdecDiscAmtK;
            //dtrGLRef["fnDiscAmt1"] = this.mdecDiscAmt;

            //dtrGLRef["fnAmt"] = this.mdecAmt;
            //dtrGLRef["fnVatAmt"] = this.mdecVatAmt;
            ...
            ////ลงรายวัน
            //if (this.mstrAtStep == SysDef.gc_ATSTEP_VOUCHER_INIT)
            //{
            //    string strGLHeadRemark = strQnCoor.Trim() + "-" + strRefNo.TrimEnd();
            //    this.pmGenVoucher(strGLHeadRemark);
            //    dtrGLRef["fcGLHead"] = this.mstr_GLHead_SaveRowID;
            //}

            this.pmSaveRefProd(inSource,ref strErrorMsg);
        ...
        private bool pmSaveRefProd(DataSet inSource,ref string ioErrorMsg)
        {
            bool bllResult = true;
            string strRowID = "";
            string strErrorMsg = "";
            ...
            foreach (DataRow dtrTemPd in inSource.Tables["QRefProd"].Rows)
            {
        ...
        private bool pmInitValueBeforeSave(DataSet inSource, ref string ioErrorMsg)
        {
            ...
            {
                dtrGLRef.Rows[0]["fcCorp"] = this.dtsDataEnv.Tables["QCorp"].Rows[0]["fcSkid"].ToString();
            ...
            if (objSQLHelper.SQLExec(ref this.dtsDataEnv, "QBranch", "BRANCH", "select fcSkid from Branch where fcCorp = ? and fcCode = ?", ref strErrorMsg))
            {
                ...
                dtrGLRef.Rows[0]["fcBranch"] = this.dtsDataEnv.Tables["QBranch"].Rows[0]["fcSkid"].ToString();
            ...
            {
                dtrGLRef.Rows[0]["fcBook"] = this.dtsDataEnv.Tables["QBook"].Rows[0]["fcSkid"].ToString();
            }
            else
            {
            ...
            if (strQcCoor.Trim() != "")
            {
                ...
                {
                    dtrGLRef.Rows[0]["fcCoor"] = this.dtsDataEnv.Tables["QCoor"].Rows[0]["fcSkid"].ToString();
                }
                else
                {
                    ...
                    if (bllResult)
                    {
                        dtrGLRef.Rows[0]["fcCoor"] = strCoorID;
                    }
                    else
                    {
                        ioErrorMsg = "ไม่พบรหัสลูกค้า " + strQcCoor + " !";
            ...
            if (strQcFrWHouse.Trim() != "")
            {
                ...
                {
                    dtrGLRef.Rows[0]["fcFrWHouse"] = this.dtsDataEnv.Tables["QWHouse"].Rows[0]["fcSkid"].ToString();
                }
                else
                {
            ...
            if (strQcToWHouse.Trim() != "")
            {
                ...
                if (objSQLHelper.SQLExec(ref this.dtsDataEnv, "QWHouse", "WHOUSE", "select fcSkid from WHOUSE where fcCorp = ? and fcBranch = ? and fcCode = ?", ref strErrorMsg))
                {
                    dtrGLRef.Rows[0]["fcToWHouse"] = this.dtsDataEnv.Tables["QWHouse"].Rows[0]["fcSkid"].ToString();
                }
                else
                {
...
#region "Load UM"
        ...
        private bool pmSyncGLRef(string inType, string inRefNo, string inSkid)
        {
            ...
            if (inType == "D")
            {
                //pobjSQLUtil_Del.SetPara(new object[] { inSkid });
                //pobjSQLUtil_Del.SQLExec("delete from crm_customerorder_item where invoice_rowid = ? ", ref strErrorMsg);
                //pobjSQLUtil_Del.SQLExec("delete from crm_customerorder where invoice_rowid = ? ", ref strErrorMsg);
                return true;
            }

            strSQLStr = " select SYNCDBDATA.CTYPE, CORP.FCCODE as QCCORP,COOR.FCCODE as QCCOOR ";
            ...
            strSQLStr += " inner join SYNCDBDATA on SYNCDBDATA.TAB_NAME = 'GLREF' and GLREF.FCSKID = SYNCDBDATA.TAB_ROWID ";
            ...
            pobjSQLUtil.SetPara(new object[] { inSkid });
