...
namespace mBudget.Business.Entity
{
    class QMasterUM
    {
        ...
        private string mstrRefChildTable = MapTable.Table.UOM;

        private string mstrConnectionString = "";
        private DBMSType mDataBaseReside = DBMSType.MSSQLServer;

        private DataSet dtsDataEnv = new DataSet(MapTable.Table.MasterUM);
        ...
        public QMasterUM(string inConnectionString, DBMSType inDataBaseReside)
        {
        ...
        private DataTable pmSearchData(object[] inPrefixPara, string inKey, string inAlias)
        {
            ...
            strSQLStr = "select " + strAppUserFldList + " from " + MapTable.Table.MasterUM + " where " + this.mstrSQLPrefix
            ...
            objSQLHelper.SQLExec(ref this.dtsDataEnv, strBrowViewAlias, MapTable.Table.MasterUM, strSQLStr, ref strErrorMsg);
        ...
        private bool pmHasUsedChild1Corp(cDBMSAgent inSQLHelper, string inRowID, ref string ioErrorMsg)
        {
            ...
            if (inSQLHelper.SQLExec(ref this.dtsDataEnv, "QChk", MapTable.Table.Product, "select fcSkid, fcCode, fcName from " + MapTable.Table.Product + " where fcUM = ?", ref strErrorMsg))
            {
