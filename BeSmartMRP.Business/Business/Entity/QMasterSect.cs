...
namespace mBudget.Business.Entity
{
    ...
    public class QMasterSect
    {
        private string mstrRefTable = MapTable.Table.MasterSect;
        private string mstrRefChildTable = MapTable.Table.Department;

        private string mstrConnectionString = "";
        private DBMSType mDataBaseReside = DBMSType.MSSQLServer;

        private DataSet dtsDataEnv = new DataSet(MapTable.Table.MasterSect);
        ...
        public QMasterSect(string inConnectionString, DBMSType inDataBaseReside)
        {
        ...
        private DataTable pmSearchData(object[] inPrefixPara, string inKey, string inAlias)
        {
            ...
            strSQLStr = "select " + strAppUserFldList + " from " + MapTable.Table.MasterSect + " where " + this.mstrSQLPrefix
            ...
            objSQLHelper.SQLExec(ref this.dtsDataEnv, strBrowViewAlias, MapTable.Table.MasterSect, strSQLStr, ref strErrorMsg);
            return this.dtsDataEnv.Tables[strBrowViewAlias].Copy();
        }

        public bool SaveChildTable(DataRow inMaster, string inOldCode, string inDeptCode)
        {
            return this.pmSaveChildTable(inMaster, inOldCode, inDeptCode);
        }
